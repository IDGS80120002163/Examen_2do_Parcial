...
export class ProductosService {
  ...
  getCategorias(): Observable<ICategoria[]> {
    return this._http.get<ICategoria[]>(`${this.apiUrl}ListaCategorias`);
  }
  ...
  update(request: IProducto): Observable<void>{
    return this._http.put<void>(`${this.apiUrl}ModificarProducto/${request.idProducto}`, request)
  }

  //Método para eliminar una tarea del endpoint EliminarProducto.
  delete(idProducto: number): Observable<void>{
    return this._http.delete<void>(`${this.apiUrl}EliminarProducto/${idProducto}`)
  }
